...
export const stake = async (yam, pid, amount, account, tokenName) => {
  let now = new Date().getTime() / 1000;
  const gas =
    GAS_LIMIT.STAKING[tokenName.toUpperCase()] || GAS_LIMIT.STAKING.DEFAULT;
  if (now >= 1597172400) {
    return yam.contracts.keyMaster.methods
      .deposit(
        pid,
...
export const unstake = async (yam, pid, amount, account) => {
  let now = new Date().getTime() / 1000;
  if (now >= 1597172400) {
    return yam.contracts.keyMaster.methods
      .withdraw(
...
export const getUni2LPContract = async (yam) => {
  return yam.contracts.uni_pair;
};

export const getKeyMasterContract = async (yam) => {
  return yam.contracts.keyMaster;
};

export const getEarned = async (yam, pid, account) => {
  const earned = new BigNumber(
    await yam.contracts.keyMaster.methods.pendingLock(pid, account).call()
  );
  return earned;
};

export const getStaked = async (yam, pid, account) => {
  const res = await yam.contracts.keyMaster.methods
    .userInfo(pid, account)
    .call();
  return yam.toBigN(res.amount);
};
...
export const getLockSupply = async (yam) => {
  console.log("yam.contracts.lockToken: ", yam.contracts.lockToken);
  return new BigNumber(
    await yam.contracts.lockToken.methods.totalSupply().call()
  );
};
